<div class="cbw-install-plugins">
	<div class="cbw-block">
		<div class="cbw-body__title"><?php
			_e( 'Install plugins', 'croco-mki' );
		?></div>
		<cbw-progress :value="progress"></cbw-progress>
		<p>
			<span v-if="'full' === action">
			<?php _e( 'Before demo data import a set of required plugins will be installed.', 'croco-mki' ); ?></span><br v-if="'full' === action">
			<span><?php _e( 'Please be patient, this may take few minutes.', 'croco-mki' ); ?></span>
		</p>
		<div
		...
		<p v-if="done && 'full' === action"><?php
			_e( 'All plugins are installed! You will be automatically redirected to the next step.', 'croco-mki' );
		?></p>
	</div>
