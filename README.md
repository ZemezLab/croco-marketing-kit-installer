...
function my_plugins_wizard_config() {
	...
	if ( ! function_exists( 'croco_mki' ) ) {
		return;
	}

	croco_mki()->settings->register_external_config( array(
		'plugins' => array(
			'plugin-1' => array(
				'name'   => esc_html__( 'Plugin 1', 'croco-mki' ),
				'sourse' => 'wordpress', // 'git', 'local', 'remote', 'wordpress' (default).
				'path'   => false, // git repository, remote URL or local path.
			),
			'plugin-2' => array(
				'name'   => esc_html__( 'Plugin 2', 'croco-mki' ),
	...
	croco_mki()->settings->register_external_config( array(
		'skins' => array(
			'skin-name' => array(
				...
				'demo'  => false,
				'thumb' => false,
				'name'  => esc_html__( 'Skin Name', 'croco-mki' ),
