...
use Croco_MKI\Tools\Files_Download as Files_Download;
...
class Module extends Module_Base {
	...
	public function enqueue_module_assets() {
		...
		wp_enqueue_script(
			'croco-mki-license',
			CROCO_MKI_URL . 'assets/js/license.js',
			array( 'cx-vue-ui' ),
			CROCO_MKI_VERSION,
			true
		);
	...
	public function verify_license() {
		...
		if ( ! $license ) {
			wp_send_json_error( array(
				'message' => esc_html__( 'Please fill in License field and try again', 'croco-mki' ),
			) );
		}
		...
		if ( ! $install_data ) {
			...
			wp_send_json_success( array(
				'message' => esc_html__( 'Your license is activated. Redirecting...', 'croco-mki' ),
			) );
		}
	...
	public function page_config( $config = array(), $subpage = '' ) {
		...
		if ( $is_active ) {
			$page_title = __( 'Crocoblock Marketing Kit installation', 'croco-mki' );
		} else {
			$page_title = __( 'Please, enter your license key to start', 'croco-mki' );
		}

		$config['title']              = __( 'Installation wizard', 'croco-mki' );
		$config['body']               = 'cbw-license';
		$config['deactivate_link']    = $this->get_deactivate_url( $config );
		$config['wrapper_css']        = 'license-panel';
		$config['button_label']       = __( 'Enter license key', 'croco-mki' );
		$config['ready_button_label'] = __( 'Start Installation', 'croco-mki' );
