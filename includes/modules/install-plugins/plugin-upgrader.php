	...
	public function maybe_adjust_source_dir( $source, $remote_source, $upgrader ) {
		...
		if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
			...
			if ( true === $wp_filesystem->move( $from_path, $to_path ) ) {
				...
				return new WP_Error(
					'rename_failed',
					esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'croco-mki' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'croco-mki' ),
			...
			return new WP_Error(
				'packaged_wrong',
				esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'croco-mki' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'croco-mki' ),
				array( 'found' => $subdir_name, 'expected' => $desired_slug )
			);
		}
