...
use Croco_MKI\Tools\Cache as Cache;
use Croco_MKI\Tools\DB_Tables as DB_Tables;
...
class Module extends Module_Base {
	...
	public function enqueue_module_assets() {
		...
		wp_enqueue_script(
			'croco-mki-content',
			CROCO_MKI_URL . 'assets/js/content.js',
			array( 'cx-vue-ui' ),
			CROCO_MKI_VERSION,
			true
		);
	...
	public function page_config( $config = array(), $subpage = '' ) {
		...
		$config['summary']          = array(
			'posts'    => __( 'Posts', 'croco-mki' ),
			'authors'  => __( 'Authors', 'croco-mki' ),
			'media'    => __( 'Media', 'croco-mki' ),
			'comments' => __( 'Comments', 'croco-mki' ),
			'terms'    => __( 'Terms', 'croco-mki' ),
			'tables'   => __( 'Custom DB Tables', 'croco-mki' ),
		);
		...
		$config['import_types']     = array(
			array(
				'value'       => 'append',
				'label'       => __( 'Append demo content to my existing content', 'croco-mki' ),
				'description' => __( 'If you have chosen this option, the sample data will be added to the current content of your website.', 'croco-mki' ),
			),
			array(
				'value'       => 'replace',
				'label'       => __( 'Replace my existing content with demo content', 'croco-mki' ),
				'description' => __( 'Choose this option if you want to install the demo content, and agree to replace your current data by the new demo content (sample data).', 'croco-mki' ),
			),
		);
	...
	public function import_chunk() {
		...
		if ( empty( $_REQUEST['chunk'] ) ) {
			...
			wp_send_json_error( array(
				'message' => esc_html__( 'Chunk number is missing in request', 'croco-mki' ),
			) );
		}
		...
		switch ( $chunk ) {
			...
			case $chunks:
				 ...
				 */
				do_action( 'croco-mki/import/finish' );
			...
			default:
				 ...
				 * Hook on last import chunk
				 */
				do_action( 'croco-mki/import/chunk', $chunk );
