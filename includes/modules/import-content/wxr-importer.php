<?php
namespace Croco_MKI\Modules\Import_Content;
...
class WXR_Importer extends \WP_Importer {
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new \WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'croco-mki' ),
							$data
						);
					}
					break;
	...
	protected function process_options( $data ) {
		...
		do_action( 'croco-mki/import/before-options-processing', $data );
		...
		do_action( 'croco-mki/import/after-options-processing', $data );
	...
	protected function maybe_clone_child_options( $option, $value ) {
		...
		$to_copy = apply_filters( 'croco-mki/import/copy-options-to-child', array(
			'theme_mods_%s',
			'%s_sidebars',
		) );
	...
	protected function process_tables( $data ) {
		...
		do_action( 'croco-mki/import/after-import-tables' );
	...
	protected function process_widgets( $data ) {
		...
		// Hook before import
		do_action( 'croco-mki/import/before-widgets-processing' );

		$data = apply_filters( 'croco-mki/import/widgets-data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( $sidebar_data = $this->sidebar_exists( $sidebar_id ) ) {
				...
				$sidebar_message      = esc_html__(
					'Sidebar does not exist in theme (using Inactive)', 'croco-mki'
				);
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = esc_html__( 'Site does not support widget', 'croco-mki' ); // explain why widget not imported
				}

				// Filter to modify settings object before conversion to array and import
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below)
				// Ideally the newer wie_widget_settings_array below will be used instead of this
				$widget = apply_filters( 'croco-mki/import/widget-settings', $widget ); // object
				...
				$widget = apply_filters( 'croco-mki/import/widget-settings-array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = esc_html__( 'Widget already exists', 'croco-mki' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					do_action( 'croco-mki/import/after-widget-import', $after_widget_import );

					// Success message
					if ( $sidebar_available ) {
						$widget_message_type = 'success';
						$widget_message = esc_html__( 'Imported', 'croco-mki' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'croco-mki' );
					}
				...
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'croco-mki' ); // show "No Title" if widget instance is untitled
		...
		// Hook after import
		do_action( 'croco-mki/import/after-process-widget', $sidebars_widgets );

		// Return results
		return apply_filters( 'croco-mki/import/widget-import-results', $results );
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		 ...
		 * @param array
		 */
		$skip_post = apply_filters( 'croco-mki/import/skip-post', false, $data );
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'croco-mki' ),
				$data['post_title'],
				$data['post_type']
			) );
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__('%s "%s" already exists.', 'croco-mki'),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice( sprintf(
					__( 'Skipping attachment "%s", fetching attachments disabled', 'croco-mki' ),
					$data['post_title']
				) );
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%s" (%s)', 'croco-mki' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'croco-mki' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'croco-mki' ),
			$original_id,
			$post_id
		) );
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"', 'croco-mki' ),
				$userdata['user_login']
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"', 'croco-mki' ),
			$userdata['user_login']
		) );

		$this->logger->debug( sprintf(
			__( 'User %d remapped to %d', 'croco-mki' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->debug( sprintf(
			__( 'Term %d remapped to %d', 'croco-mki' ),
			$original_id,
			$term_id
		) );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new \WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'croco-mki' ) );
		}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( 0 == $filesize ) {
			...
			return new \WP_Error(
				'import_file_error',
				__( 'Zero size file downloaded', 'croco-mki' )
			);
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new \WP_Error(
				'import_file_error',
				sprintf( __( 'Remote file is too large, limit is %s', 'croco-mki' ), size_format( $max_size ) )
			);
		}
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new \WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'croco-mki' ) );
		}
