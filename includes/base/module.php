...
abstract class Module {
	...
	public function __construct() {
		...
		add_action(
			'croco-mki/dashboard/before-enqueue-assets/' . $this->get_slug(),
			array( $this, 'assets' )
		);

		add_action( 'wp_ajax_croco-mki/' . $this->get_slug(), array( $this, 'process_ajax' ) );
	...
	public function assets() {
		...
		add_filter( 'croco-mki/dashboard/js-page-config', array( $this, 'page_config' ), 10, 2 );
		add_filter( 'croco-mki/dashboard/js-page-templates', array( $this, 'page_templates' ), 10, 2 );
	...
	public function process_ajax() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array(
				'message' => __( 'You are not allowed to access this', 'croco-mki' ),
			) );
		}
		...
		if ( ! $nonce || ! wp_verify_nonce( $nonce, Plugin::instance()->dashboard->page_slug ) ) {
			wp_send_json_error( array(
				'message' => __( 'Nonce verfictaion failed', 'croco-mki' ),
			) );
		}
