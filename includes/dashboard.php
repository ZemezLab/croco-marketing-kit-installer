...
class Dashboard {
	...
	public $page_slug = 'croco-mki';

	public function __construct() {
		add_action( 'admin_menu', array( $this, 'register_menu_page' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'assets' ) );
		add_action( 'admin_init', array( $this, 'maybe_redirect_after_activation' ) );
		add_filter( 'plugin_action_links_' . CROCO_MKI_PLUGIN_BASE, array( $this, 'plugin_action_links' ) );
	}
	...
	public function maybe_redirect_after_activation() {
		...
		if ( ! get_transient( 'croco_mki_redirect' ) ) {
			return;
		}
		...
		delete_transient( 'croco_mki_redirect' );
	...
	public function register_menu_page() {
		...
		add_management_page(
			__( 'Crocoblock Marketing Kit', 'croco-mki' ),
			__( 'Crocoblock Marketing Kit', 'croco-mki' ),
			'manage_options',
			$this->page_slug,
	...
	public function assets( $hook ) {
		...
		require_once CROCO_MKI_PATH . 'framework/vue-ui/cherry-x-vue-ui.php';

		$ui = new \CX_Vue_UI( array(
			'url'  => CROCO_MKI_URL . 'framework/vue-ui/',
			'path' => CROCO_MKI_PATH . 'framework/vue-ui/',
		) );
		...
		wp_register_script(
			'croco-mki-mixins',
			CROCO_MKI_URL . 'assets/js/mixins.js',
			array( 'cx-vue-ui' ),
			CROCO_MKI_VERSION,
			true
		);
		 ...
		 */
		do_action( 'croco-mki/dashboard/before-enqueue-assets', $this );

		/**
		 * Fires before enqueue page assets with dynamic subpage name
		 */
		do_action( 'croco-mki/dashboard/before-enqueue-assets/' . $this->get_subpage(), $this );

		wp_enqueue_script(
			'croco-mki',
			CROCO_MKI_URL . 'assets/js/wizard.js',
			array( 'cx-vue-ui' ),
			CROCO_MKI_VERSION,
			true
		);
		...
		wp_localize_script(
			'croco-mki',
			'CBWPageConfig',
			apply_filters( 'croco-mki/dashboard/js-page-config', array(
				'title'        => __( 'Installation Wizard', 'croco-mki' ),
		...
		wp_enqueue_style(
			'croco-mki',
			CROCO_MKI_URL . 'assets/css/wizard.css',
			array(),
			CROCO_MKI_VERSION
		);
	...
	public function print_js_templates() {
		...
		$templates = apply_filters(
			'croco-mki/dashboard/js-page-templates',
			array(
				'main'         => 'common/main',
