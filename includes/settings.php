...
class Settings {
	...
	public function maybe_update_remote_data( $settings ) {
		...
		if ( ! empty( $settings['plugins']['get_from'] ) ) {
			$settings['plugins'] = $this->get_remote_data( $settings['plugins']['get_from'], 'croco_mki_plugins' );
		}

		if ( ! empty( $settings['skins']['get_from'] ) ) {
			$settings['skins'] = $this->get_remote_data( $settings['skins']['get_from'], 'croco_mki_skins' );
		}
	...
	public function clear_transient_data() {
		set_site_transient( 'croco_mki_plugins', null );
		set_site_transient( 'croco_mki_skins', null );
	}
	...
	public function get_defaults( $part = null ) {
		...
		if ( null === $this->defaults ) {
			...
			$plugins = array(
				'get_from' => 'https://account.crocoblock.com/wp-content/uploads/static/mk-wizard-plugins.json',
			);

			$skins = array(
				'get_from' => 'https://account.crocoblock.com/wp-content/uploads/static/mk-wizard-skins.json',
			);
